...
class EditCourseController: UIViewController,UITextFieldDelegate {
    ...
    @IBAction func editCourse() {
        ...
        request.POST("http://localhost:8080/seven/edit_course.action", parameters: params, completionHandler: {(response: HTTPResponse) in
            if let res: AnyObject = response.responseObject {
                let data = ResponseData(JSONDecoder(res))
                print("result:\(data.result!)")
                if (data.result! == 1) {
                    print("editCourse secceed")
                } else {
                    print("editCourse fail")
                }
            }
        })
    }
