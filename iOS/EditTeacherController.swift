...
class EditTeacherController: UIViewController, UITextFieldDelegate {
    struct JSON : JSONJoy {
        var result: Int?
        init() {
        }
    ...
    @IBAction func editTeacher() {
        ...
        request.POST("http://localhost:8080/seven/edit_teacher.action", parameters: params, completionHandler: {(response: HTTPResponse) in
            if let res: AnyObject = response.responseObject {
                let json = JSON(JSONDecoder(res))
                print("result: \(json.result!)")
                if (json.result! == 1) {
                    print("edit teacher succeed!")
                } else {
                    print("edit teacher failed!")
                }
            }
        })
    }
